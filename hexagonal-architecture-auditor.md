...
description: >-
  ...
  - <example>
    ...
    user: "audit l'architecture de src/ et livre le rapport"
    assistant: "I'll use the hexagonal-architecture-auditor agent to run the audit and deliver the compliance report."
    <commentary>
    The user wants an architecture audit plus report delivery, so use the hexagonal-architecture-auditor agent.
    </commentary>
    </example>
...
3. **Si un `taskId` est fourni dans ton contexte** (orchestration par l'agent
   ...
   via le MCP `task-orchestrator` — c'est ce qui déclenche la notification :
       ...
       task_event(taskId="<taskId>", type="AUDIT_COMPLETED", by="hexagonal-architecture-auditor", detail={"auditId": "<auditId>", "zip": "audits/audit-arch-<...>.zip", "level": "...", "score": "..."})
